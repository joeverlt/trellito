...
export const listsSlice = createSlice({
  ...
  reducers: {
    ...
    sortList: (
      state,
      action: PayloadAction<{ from: number; to: number; board: string }>
    ) => {
      const { from, to, board } = action.payload
      const lists = [...state.data.filter((list) => list.board == board)]
      const otherLists = [...state.data.filter((list) => list.board != board)]
